...
export const BroadcastAlert = () => {
  ...
  // Replace with actual enavId source (e.g., from auth/user context)
  const enavId = import.meta.env.VITE_ENAV_ID;

  const handleBroadcast = async () => {
    const result = await postAnnouncement(message, enavId);
    if (result) {
      setMessage('');
    }
  };
