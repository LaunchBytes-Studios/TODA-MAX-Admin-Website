...
import { API_BASE_URL } from '@/utils/reward.utils'; // reuse if shared
...
export function useGetChatSessions() {
  ...
  const getChatSessions = async () => {
    try {
      ...
      const response = await axios.get<ApiResponse<ChatSessionWithPatient[]>>(
        `${API_BASE_URL}/enavigator/supportChat/chat-sessions`,
        { headers },
      );
      ...
      const errorMessage =
        axiosError.response?.data?.message || 'Failed to fetch chat sessions';
  ...
  return { getChatSessions, loading, error };
}
